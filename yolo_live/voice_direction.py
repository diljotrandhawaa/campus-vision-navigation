...
"""Select and reacquire detections by target class, then give camera directions."""
...
class TargetController(AlignmentController):
    ...
    def update(self, detections, now):
        ...
        # Keep the optional left/center/right selection.
        if self.horizontal is not None:
            candidates = [
                (index, detection)
                for index, detection in candidates
        ...
        # Continue with the closest box when possible. If it cannot be matched,
        # reacquire the highest-confidence detection of the selected class.
        selected = None
        if self.previous_box is not None:
            ...
            for index, detection in candidates:
                ...
                if distance < 0.20:
                    nearby_candidates.append((distance, index, detection))

            if nearby_candidates:
                _, index, detection = min(nearby_candidates)
        ...
        if selected is None:
            ...
            # Tell AlignmentController this is a reacquired class match.
            self.previous_box = None
            self.smoothed_offset = None
            self.state = "lost"

        index, detection = selected
        result = super().update([detection], now)
        result.update(
            target_index=index,
            matches=len(candidates),
